...
st.title("🛒 Jornada de Compra — Projetos")
st.markdown(
        ...
        with st.container(border=True):
            ...
            with c_header:
                ...
                if proj.get("categoria"):
                    meta_parts.append(f"🏷️ **Categoria:** {proj['categoria']}")
                meta_parts.append(f"📊 **Datasets:** {proj['total_datasets']}")
                meta_parts.append(f"🗂️ **Entrevistas:** {proj['total_interviews']}")
            ...
            with c_actions:
                ...
                with st.popover("⚙️ Opções", use_container_width=True):
                    if st.button("✏️ Editar Briefing", key=f"edit_jc_{p_id}", use_container_width=True):
                        st.session_state["jc_project_id"] = p_id
                        st.switch_page("modules/jornada_compra/preparacao.py")
                    
                    if st.button("🗑️ Excluir Projeto", key=f"del_jc_{p_id}", use_container_width=True):
